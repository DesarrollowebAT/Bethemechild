				...
				if( $paged < $total ){
					$output .= '<div class="column one pager_wrapper pager_lm">';
						$output .= '<a class="pager_load_more button button_js" href="'. get_pagenum_link( $next ) .'">';
							...
							$output .= '<span class="button_label">Ver más</span>';
						$output .= '</a>';
					$output .= '</div>';
				}
				...
				$output .= '<div class="column one pager_wrapper">';
					$output .= '<div class="pager">';
						...
						if( $paged >1 ){
							...
							$output .= '<a class="prev_page" href="'. get_pagenum_link( $prev ) .'"><i class="icon-left-open"></i>Anterior</a>';
						}
						...
						if( $paged < $total ){
							...
							$output .= '<a class="next_page" href="'. get_pagenum_link( $next ) .'">Siguiente<i class="icon-right-open"></i></a>';
						}
		...
		if( is_single( $postID ) ){
			...
			if( get_post_type() == 'post' ){
				...
				if( mfn_opts_get( 'mfn-seo-schema-type' ) ){
                    ...
                    $link_after_schema .= '<div class="metas">';
                    $link_after_schema .= '<div itemscope="" itemprop="mainEntityOfPage" itemtype="https://schema.org/WebPage">';
                    $link_after_schema .= '<meta itemprop="url" content="http://www.academiadeltransportista.com">';
                    $link_after_schema .= '<meta itemprop="name" content="Academia del Transportista">';
                    $link_after_schema .= '</div>';
                    $link_after_schema .= '<meta itemprop="headline" content="'. get_the_title() .'">';
                    $link_after_schema .= '<div itemprop="author" itemscope="" itemtype="https://schema.org/Person" class="">';
                    $link_after_schema .= '<meta itemprop="name" content="'. get_the_author() .'">';
                    $link_after_schema .= '</div>';
                    $link_after_schema .= '<div itemprop="publisher" itemscope itemtype="https://schema.org/Organization">';

                    $publisher_name = get_bloginfo('name');
                    $publisher_logo_url = '';
                    $publisher_logo_width = 0;
                    $publisher_logo_height = 0;

                    $publisher_logo = wp_get_attachment_image_src( 7753 , 'full' );

                    $publisher_logo_url = $publisher_logo[0];
                    $publisher_logo_width = $publisher_logo[1];
                    $publisher_logo_height = $publisher_logo[2]; 

                    $link_after_schema .= '<meta itemprop="name" content="'. $publisher_name .'">';
                    $link_after_schema .= '<div itemprop="logo" itemscope itemtype="https://schema.org/ImageObject">';
                    $link_after_schema .= '<meta itemprop="url" content="'. $publisher_logo_url .'">';
                    $link_after_schema .= '<meta itemprop="width" content="'. $publisher_logo_width .'">';
                    $link_after_schema .= '<meta itemprop="height" content="'. $publisher_logo_height .'">';
                    $link_after_schema .= '</div>';
                    $link_after_schema .= '</div>';
                    ...
                    $link_after_schema .= '<meta itemprop="datePublished" content="'. $datePublished .'"/>';
                    $link_after_schema .= '<meta itemprop="dateModified" content="'. $dateModified .'"/>';
                    $link_after_schema .= '</div>';
